<template>
  <div class="apply__page">
    ...
    <div class='hack__team'>
      <label class='hack__label'>List of preferred team members (Up to 4)</label>
      <Input
        class='hack__member'
        placeholder='You'
        name='you'
      ...
      />
      <div
        v-for='(member, i) in team_members_'
        class='hack__item'
        :key='i'
      >
        <Input
          ...
          :name='"member-" + i'
        />
        <Button icon='trash' :click='() => team_members_.splice(i, 1)' secondary/>
      </div>
      <Button icon='plus' :click='add' :disabled='team_members_.length > 2'>Add a team member</Button>
    </div>
  </div>
</template>
    ...
    methods: {
      check() {
        this.$emit('update:valid', (
          ...
          this.team_members_.reduce((acc, curr) => (
            acc && curr.length > 0
          ), true)
        ));
      },
